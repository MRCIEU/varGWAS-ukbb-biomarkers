...
option_list = list(
  ...
  make_option(c("-h", "--hit"), type="character", default=NULL, help="Conditional hit", metavar="character")
);
...
for (snp in snps){
    if (!is.null(opt$h)){
        f <- paste0(opt$trait, " ~ ", snp, " * ", opt$modifier, " + ", opt$h, " + age_at_recruitment.21022.0.0 + sex.31.0.0 + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10")
    } else {
